...
mean_HR_LCW = mean(heartRateData_lcw);
mean_RR_LCW = mean(respRateData_lcw);


disp('Average Heart Rate for LCW: '+ mean_HR_LCW);
disp('Average Resp Rate for LCW: '+ mean_RR_LCW);
...
mean_HR_HCW = mean(heartRateData_hcw);
mean_RR_HCW = mean(respRateData_hcw);


disp('Average Heart Rate for HCW: '+ mean_HR_HCW);
disp('Average Resp Rate for HCW: '+ mean_RR_HCW);
...
disp("Average Reaction Time for LCW: " + mean(Tr_LCW));
disp("Average Reaction Time for HCW: " + mean(Tr_HCW));
...
for Gain = 10000:10000:100000
    ...
    for InitSpeed = 20:40
        ...
        disp("Stopping Distance = " + lastDistance_lcw);
        disp("Stopping Time = " + lastTime_lcw);

        % Importing the VehicleModelOnly.slx file to calculate the Action Time (Ta)
        % LCW
        set_param('HumanActionModel/Step', 'Time', num2str(curr_Tr_LCW));
        ...
        disp("Stopping Distance = " + lastDistance_hcw);
        disp("Stopping Time = " + lastTime_hcw);


        % Importing the VehicleModelOnly.slx file to calculate the Action Time (Ta)
        % HCW
        set_param('HumanActionModel/Step', 'Time', num2str(curr_Tr_HCW));
