...
factor = 0.01;
totalCollision_LCW = inf;
totalCollision_HCW = inf;
overallCollision = inf;
bestFactor = 0.01;
switchCount_lcw=0;
switchCount_hcw=0;
...
while factor<0.1
    ...
    for i = 1:100
        ...
        if roadConditions(i)==1
            ...
            disp("Stopping Distance = " + lastDistance);
            disp("Stopping Time = " + lastTime);
            ...
            if lastDistance >= 0
                if hStop < lastTime
                    Tr_3 = rq_lcw(i)*0.01;
                    set_param('HumanActionModel/Step', 'Time', num2str(Tr_3));
                    set_param('HumanActionModel/Step', 'After', num2str(decelLim_lcw*1.1));
                    simModel_new_lcw = sim('HumanActionModel.slx');
            ...
            lastTime = simModel_hcw.sx1.Time(end);
    ...
    if collision < overallCollision
        totalCollision_LCW = collision_LCW;
        totalCollision_HCW = collision_HCW;
        overallCollision = collision;
        bestFactor = factor;
...
disp("Total Minimum Collision = "+ overallCollision);
disp("Total Collision for LCW = "+ totalCollision_LCW);
disp("Total Collision for HCW = "+ totalCollision_HCW);
